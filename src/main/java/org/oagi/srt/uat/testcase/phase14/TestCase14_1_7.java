...
public class TestCase14_1_7 {
    ...
    @Test
    public void testAdminUserCanSeeAllDetailsOfContextCategories() {
        login(webDriver, enterpriseAdmin_1);
        ...
        WebElement link = searchContextCategoryByName(webDriver, ctxCatName_1);
        link.click();
        assertCanSeeDetails();

        link = searchContextCategoryByName(webDriver, ctxCatName_2);
        link.click();
        assertCanSeeDetails();

        link = searchContextCategoryByName(webDriver, ctxCatName_3);
        link.click();
        assertCanSeeDetails();

        link = searchContextCategoryByName(webDriver, ctxCatName_4);
        link.click();
        assertCanSeeDetails();
    }

    private void assertCanSeeDetails() {
        assertNotNull(findElementByContainingId(webDriver, "input[type=text]", "name"));
        assertNotNull(webDriver.findElement(By.cssSelector("textarea")));
        assertNotNull(findElementByText(webDriver, "button[type=submit]", "Update"));
        assertNotNull(findElementByText(webDriver, "button[type=submit]", "Discard"));
    }
